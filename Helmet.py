...
def getOutputsNames(net):
    ...
    return [layersNames[int(i) - 1] for i in net.getUnconnectedOutLayers().flatten()]  # Fixed indexing issue
...
def postprocess(frame, outs, option):
    ...
    count_person = 0  # Counter for detected helmets
    if len(indices) > 0:  # Ensure indices is not empty
        for i in indices.flatten():  # Convert to a list of integers
            box = boxes[i]
            left, top, width, height = box
            frame_count_out = drawPred(classIds[i], confidences[i], left, top, left + width, top + height, frame, option)
            
            my_class = 'Helmet'
            unknown_class = classes[classIds[i]]  # Fixed class indexing
